...
#define J1939_MAKE_ID(prio, dp, pf, ps, sa)                                    \
  (((uint32_t)(prio & 0x7) << J1939_PRIORITY_SHIFT) |                          \
   ...
   ((uint32_t)(ps & 0xFF) << J1939_PS_SHIFT) | ((uint32_t)(sa & 0xFF)))
...
#define J1939_PF_ADDRESS_CLAIMED                                               \
  0xEE // 238 (>=240? No, 238 < 240, but J1939 uses 254/255 for network
       // management)
...
public:
  ...
  bool send(uint8_t pf, uint8_t ps, const uint8_t *data, uint8_t len,
            uint8_t priority = 6);
  bool sendBroadcast(uint8_t pf, const uint8_t *data, uint8_t len,
                     uint8_t priority = 6);
